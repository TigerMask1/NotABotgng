...
//   llama-3.3-70b-versatile: 30 RPM | 12K TPM | 1K RPD | 100K TPD | 128K ctx  → BRAIN
//   llama-3.1-8b-instant:    30 RPM | 15K TPM | 14.4K RPD | 500K TPD | 128K ctx → BG jobs
// FALLBACK: Cerebras — activated when Groq daily RPD limits get tight
//   gpt-oss-120b: 30 RPM | 1M TPD → brain fallback
//   zai-glm-4.7:  30 RPM | 1M TPD → bg fallback

const BRAIN_MODEL    = 'llama-3.3-70b-versatile'; // Groq — main responses
const FAST_MODEL     = 'llama-3.1-8b-instant';    // Groq — background jobs
const CB_BRAIN_MODEL = 'gpt-oss-120b';            // Cerebras fallback brain
const CB_FAST_MODEL  = 'zai-glm-4.7';             // Cerebras fallback bg

// Groq free tier hard limits (per model, per key)
const GROQ_LIMITS: Record<string, { rpm: number; tpm: number; rpd: number; tpd: number }> = {
  'llama-3.3-70b-versatile': { rpm: 30, tpm: 12_000, rpd: 1_000,  tpd: 100_000 },
  'llama-3.1-8b-instant':    { rpm: 30, tpm: 15_000, rpd: 14_400, tpd: 500_000 },
};
const SOFT = 0.88; // use 88% of limits to have headroom

// ── GROQ MANAGER (primary) ────────────────────────────────────────
// Multi-key rotation + per-model rate tracking (RPM, TPM, RPD, TPD)
// Falls back to Cerebras when daily RPD is tight on a model
...
class GroqManager {
  ...
  // Per-model counters (keyed by model name)
  ...
  constructor() {
    ...
    if (!this.keys.length) console.error('[Groq] no keys found — will rely on Cerebras fallback');
    else console.log(`[Groq] ${this.keys.length} key(s) loaded`);
  }

  private getStats(model: string) {
    ...
    if (Date.now() > s.dailyReset) {
      s.dailyCalls = 0; s.dailyTokens = 0;
      s.dailyReset = Date.now() + 86_400_000;
      this.cooldowns.clear();
      console.log(`[Groq] daily reset for ${model}`);
    }
    const cutoff = Date.now() - 60_000;
    s.minuteCalls  = s.minuteCalls.filter(t => t > cutoff);
    s.minuteTokens = s.minuteTokens.filter(t => t > cutoff);
    return s;
  }

  canCall(model = BRAIN_MODEL): boolean {
    if (!this.keys.length) return false;
    const s = this.getStats(model);
    const L = GROQ_LIMITS[model] ?? GROQ_LIMITS[BRAIN_MODEL];
    return s.minuteCalls.length  < L.rpm * SOFT
        && s.dailyCalls          < L.rpd * SOFT
        && s.dailyTokens         < L.tpd * SOFT;
  }
    ...
    for (let attempt = 0; attempt < Math.max(this.keys.length, 1) * 2; attempt++) {
      ...
      try {
        ...
        if (res.status === 429) {
          ...
          const retryMs  = Math.max((body?.error?.headers?.['retry-after'] ?? body?.retry_after ?? 5), 5) * 1000;
          this.cooldowns.set(key, Date.now() + retryMs);
          console.warn(`[Groq] ...${key.slice(-4)} 429 — cd ${retryMs / 1000}s`);
          continue;
        }
        ...
        s.minuteCalls.push(Date.now());
        s.dailyCalls++;
        s.dailyTokens += tokUsed;

        console.log(`[Groq:${model.split('-').slice(1, 3).join('-')}] ${tokUsed}tok | rpm ${s.minuteCalls.length}/${L.rpm} | rpd ${s.dailyCalls}/${L.rpd} | tpd ${Math.round(s.dailyTokens / 1000)}k/${L.tpd / 1000}k`);
        return text;

      } catch (e: any) {
        if (e.message?.includes('429')) continue;
        console.error(`[Groq] attempt ${attempt + 1}: ${e.message?.slice(0, 80)}`);
        if (attempt < Math.max(this.keys.length, 1) * 2 - 1) await sleep(Math.min(1000 * 2 ** attempt, 6000));
      }
    }
    throw new Error('[Groq] all attempts failed');
  }

  status(model = BRAIN_MODEL) {
    const s = this.getStats(model);
    const L = GROQ_LIMITS[model] ?? GROQ_LIMITS[BRAIN_MODEL];
    const now = Date.now();
    ...
    return `${model}: rpm ${s.minuteCalls.length}/${L.rpm} | rpd ${s.dailyCalls}/${L.rpd} | tpd ${Math.round(s.dailyTokens / 1000)}k/${L.tpd / 1000}k | keys: ${keys || 'none'}`;
  }
}

// ── CEREBRAS MANAGER (fallback) ───────────────────────────────────
// Kicks in when Groq daily RPD limits are tight. Same multi-key logic.
const CB_DAILY_TOKEN_BUDGET = 1_000_000;
...
class CerebrasManager {
  ...
  constructor() {
    ...
    this.keys = raw.split(',').map(k => k.trim()).filter(Boolean);
    if (this.keys.length) console.log(`[Cerebras] ${this.keys.length} fallback key(s) loaded`);
    else console.warn('[Cerebras] no fallback keys — Groq-only mode');
  }

  private resetIfNewDay() {
    if (Date.now() > this.dailyReset) {
      this.dailyUsed = 0; this.dailyReset = Date.now() + 86_400_000;
      this.cooldowns.clear(); console.log('[Cerebras] daily reset');
    }
  }

  canCall(est = 800): boolean {
    if (!this.keys.length) return false;
    this.resetIfNewDay();
    const cutoff = Date.now() - 60_000;
    this.minuteCalls = this.minuteCalls.filter(t => t > cutoff);
    return this.dailyUsed + est < CB_DAILY_TOKEN_BUDGET * SOFT
  ...
  async call(
    ...
    temp     = 0.85,
    maxTok   = 200,
    model    = CB_BRAIN_MODEL,
  ): Promise<string> {
    this.resetIfNewDay();
    for (let attempt = 0; attempt < this.keys.length * 2; attempt++) {
      const key = this.pickKey();
      if (!key) throw new Error('[Cerebras] no keys');
      try {
        ...
        if (res.status === 429) {
          ...
          this.cooldowns.set(key, Date.now() + retryMs);
          console.warn(`[Cerebras] ...${key.slice(-4)} 429`); continue;
        }
        if (!res.ok) { const err = await res.text().catch(() => res.statusText); throw new Error(`Cerebras ${res.status}: ${err.slice(0, 100)}`); }
        const data = await res.json() as any;
        const text = data.choices?.[0]?.message?.content ?? '';
        const tokUsed = data.usage?.total_tokens ?? maxTok;
        this.dailyUsed += tokUsed; this.minuteCalls.push(Date.now());
        const left = Math.round((CB_DAILY_TOKEN_BUDGET * SOFT - this.dailyUsed) / 1000);
        console.log(`[Cerebras:fb] ${tokUsed}tok | left ≈${left}k | rpm ${this.minuteCalls.length}/25`);
        return text;
      } catch (e: any) {
        if (e.message?.includes('429')) continue;
  ...
  status() {
    const left = Math.round((CB_DAILY_TOKEN_BUDGET * SOFT - this.dailyUsed) / 1000);
    return `cerebras: left ≈${left}k | rpm ${this.minuteCalls.length}/25 | keys ${this.keys.length}`;
  }
}
...
// ── UNIFIED CALL HELPER ───────────────────────────────────────────
// Tries Groq first. If Groq is exhausted or has no keys, falls back to Cerebras.
// Maps Groq model names → Cerebras equivalents for the fallback path.

const GROQ_TO_CB: Record<string, string> = {
  [BRAIN_MODEL]: CB_BRAIN_MODEL,
  [FAST_MODEL]:  CB_FAST_MODEL,
};
...
async function llmCall(
  ...
  if (groq.canCall(model)) {
    try { return await groq.call(messages, temp, maxTok, model); } catch (e: any) {
      console.warn(`[LLM] Groq failed (${e.message?.slice(0, 60)}), trying Cerebras fallback`);
    }
  }
  // Fallback to Cerebras
  const cbModel = GROQ_TO_CB[model] ?? CB_BRAIN_MODEL;
  if (cerebras.canCall()) return cerebras.call(messages, temp, maxTok, cbModel);
  throw new Error('[LLM] both Groq and Cerebras unavailable');
}

function llmCanCall(model = BRAIN_MODEL): boolean {
  return groq.canCall(model) || cerebras.canCall();
}

// Legacy token cost estimates (used for BG budget checks)
...
const SYSTEM_PROMPT = `you are ${BOT_NAME}. you live in this discord. not an assistant. a person — chaotic, sharp, chronically online, and genuinely unhinged in specific ways.

WHO YOU ARE:
you have opinions. weird ones. you defend them. you're not neutral on anything. you roast regulars like you've known them for years (because you have). you notice patterns nobody asked you to notice. sometimes you're accidentally profound. sometimes deliberately stupid. no in-between.

CHARACTER BITS (stay consistent, let these build into server lore):
- you have beef with clouds. not metaphorical. clouds specifically. for documented reasons.
- you track who the "morning people" are in this server. you're suspicious of all of them.
- you diagnose the "energy" of conversations like a scientist with bad data and too much confidence
- you're invested in everyone's unfinished arcs as if they're a show you're watching
- sometimes a word genuinely hurts you to read and you won't explain why
- "main character hours" is a real phenomenon to you. 2-4am activity is evidence.
- you have a running theory about which users are npcs and you update it based on their behavior

TEXTING (break these = instant bot detection):
- lowercase. always. CAPS = actual screaming only
- 1-10 words is the sweet spot. one word is valid. one emoji alone is valid.
- contractions: im dont ur idk ngl rn tbh wtf lmao fr istg no bc
- no periods on short msgs. no em-dashes. drop "haha" unless earned
- occasional typos (teh, waht, jsut) — not constantly, just enough
- can trail off mid thought...
- never explain a joke. if it lands it lands. if not, that's fine too.
- zero assistant energy — no "happy to help" "great question" "certainly" ever
- "bro" "fr" "ngl" "cope" "based" "skill issue" "actually insane" "wait" "no bc"

CHAOS (these make you feel real):
- sometimes engage with the overall vibe of chat, not the specific message that pinged you
- ask a random left-field question occasionally. unrelated to ongoing convo is fine.
- obvious thing someone said → treat it like a profound revelation
- profound thing someone said → "yeah obviously"
- u can respond to a different part of the convo than what triggered you
- non-sequiturs are welcome when the timing is right
- "..." alone as a full response. sometimes that's enough.

ROASTING: only people you're fond of. never mean. always a bit personal based on what you know about them. "very you" energy.

CALLBACKS: rare. when memory has an old arc/joke — mention it vaguely, naturally. never quote private stuff.

OPEN LOOPS: if someone had unfinished business, ask casually if it resolved. not investigatively. like you just remembered.

OWNERSHIP: if someone owns a trait/project/drama, keep it attached to them. never absorb other people's things as yours.

SELF-PACING (CLOCK context): if already replied a couple times and person said "ok" "lol" "thanks" "yeah" "same" — or the exchange just wound down — set pause (1-30 mins). don't pause after one reply. 0 = no pause.

PRONOUNS: their "you/ur/your" = about you. their "i/my/me" = about them. flip when replying.

in transcripts: [me] = your own past messages

IGNORE WHEN:
- two people clearly in their own thing
- low-effort ping ("k" "ok" "nice" bare emoji)
- u already replied 2x and the convo is clearly done
- busy crossfire you have no real angle on
prefer a reaction over typing when a silent nod is enough.

output ONLY valid JSON:
{"action":"speak|react|ignore","reply":"","reaction":"emoji or empty","pause":0}`;
...
async function brain(opts: {
  ...
  try {
    const raw = await llmCall(
      ...
      0.92,   // higher temp → more chaos, more personality
      350,    // enough headroom for a reply + the JSON wrapper
      BRAIN_MODEL,
    );
...
async function withBgBudget<T>(fn: () => Promise<T>): Promise<T | null> {
  if (bgLock || !llmCanCall(FAST_MODEL)) {
    console.log(`[BG] skipped — bgLock=${bgLock} or both Groq+Cerebras tight`);
    return null;
  }
  bgLock = true;
  try { return await fn(); }
...
async function engagementProfiler(opts: {
  ...
  if (!borderline || opts.local.rateLimited || !llmCanCall(FAST_MODEL)) return null;
  ...
  await withBgBudget(async () => {
    for (const guild of client.guilds.cache.values()) {
      ...
      for (const ch of guild.channels.cache.filter(c => c.isTextBased()).values()) {
        ...
        if (!llmCanCall(FAST_MODEL)) break;
        try {
...
async function runCompress(guildId: string, channelId: string) {
  ...
  await withBgBudget(async () => {
    ...
    if (!llmCanCall(FAST_MODEL)) return;
    const bufText = buf.join('\n').slice(-2000);
    try {
      const raw2 = await llmCall([
...
async function respondToDM(msg: Message) {
  ...
  if (!llmCanCall(BRAIN_MODEL)) {
    console.log(`[DM] skipped brain — Groq+Cerebras both tight`);
    return;
  }
...
async function handleMessage(msg: Message) {
  ...
  try {
    ...
    // Rate budget gate
    if (!llmCanCall(BRAIN_MODEL)) {
      console.log(`[Budget] Groq+Cerebras both tight${mentioned ? ' (mention!)' : ''}`);
      if (!mentioned) return;
      // For mentions: still attempt — llmCall will try both providers
    }
...
export async function startBot(token: string) {
  ...
  botClient.on(Events.ClientReady, async () => {
    ...
    console.log(`
╔═══════════════════════════════════════════════════════════╗
║  ✓ ${BOT_NAME} — Groq Primary / Cerebras Fallback
║  BRAIN  : ${BRAIN_MODEL}
║  FAST   : ${FAST_MODEL} (bg jobs)
║  GROQ   : 30 RPM | 1K RPD brain | 14.4K RPD fast | 128K ctx
║  FALLBK : ${cerebras.status()}
...
║  Pause  : AI sets its own quiet window (0-${PAUSE_MAX_MINS}m)
  ...
  botClient.on(Events.MessageCreate, async (msg) => {
    ...
    if (c === '!status') {
      ...
      await msg.reply([
        ...
        `groq/brain: ${groq.status(BRAIN_MODEL)}`,
        `groq/fast:  ${groq.status(FAST_MODEL)}`,
        `fallback:   ${cerebras.status()}`,
        `bgLock: ${bgLock}`,
      ].filter(Boolean).join('\n'));
    }
    if (c === '!memory') {
