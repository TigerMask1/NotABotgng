...
function goActive(channelId: string, mins = ACTIVE_MINS, reason = '') {
  ...
  if (!prev || prev.mode !== 'active') activityClocks.set(channelId, { activeSince: Date.now(), replies: 0 });
...
async function runProactive(client: Client) {
  ...
  await withBgBudget(async () => {
    ...
    for (const [channelId, msgs] of stmStore.entries()) {
      ...
      candidates.push({ id: channelId, hint: `quiet ${Math.round(idle / 60_000)}m. last: "${msgs[msgs.length - 1].content.slice(0, 50)}"` });
    }
...
export async function startBot(token: string) {
  ...
  botClient.on(Events.ClientReady, async () => {
    ...
    setTimeout(() => {
      ...
      setInterval(() => { if (!botClient) return; runProfiler(botClient).catch(() => {}); }, PROFILER_INTERVAL);
    }, 60 * 60_000);
